...
from .yolov5_ultra import YOLOv5
...
class Yolov5Node(Node):
    def __init__(self, name):
        ...
        iou_thresh = self.get_parameter('iou').value
        self.get_logger().info('\033[1;32m%s\033[0m' % str(conf_thresh))
        
        self.classes = self.get_parameter('classes').value

        package_share = get_package_share_directory("yolov5_ros2")
        model_path = os.path.join(package_share, "models", "yolov5nu.pt")
        
        # self.yolov5 = YoLov5TRT(os.path.join(MODE_PATH, engine), os.path.join(MODE_PATH, lib), self.classes, conf_thresh)
        # self.yolov5 = YOLOv5("yolov5n.pt", self.classes, conf_thresh, iou_thresh)
        self.yolov5 = YOLOv5(model_path, self.classes, conf_thresh, iou_thresh)
        ...
        while self.running:
            ...
            try:
                if self.start:
                    objects_info = []
                    h, w = image.shape[:2]
                    boxes, scores, classid = self.yolov5.predict(image)
                    for box, cls_conf, cls_id in zip(boxes, scores, classid):
                        class_name = self.yolov5.model.names[cls_id]
